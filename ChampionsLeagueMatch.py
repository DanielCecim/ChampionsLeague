...
class Shop:
    def __init__(self, stock, queue_max, cashiers, db_manager):
    ...
    def buy(self, fan, item, price): # Fan tries to buy an item
        with self.cashiers: # Wait for a cashier availability
            with self.stock_lock: # Lock stock access
                ...
                if self.db_manager:
                    self.db_manager.log_sale(fan.name, item, price, shop_type="Shop")
                return True

# Create food and merch shops using list comprehension
food_shops = [Shop(shop["stock"], SHOP_QUEUE_MAX, CASHIERS, db_manager) for shop in FOOD_SHOPS]
merch_shops = [Shop(shop["stock"], SHOP_QUEUE_MAX, CASHIERS, db_manager) for shop in MERCH_SHOPS]
